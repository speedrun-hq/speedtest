...
export class EvmClient {
   ...
   */
  async approveToken(tokenAddress: string, amount: bigint): Promise<ethers.TransactionReceipt> {
    const tokenContract = new ethers.Contract(tokenAddress, erc20Abi, this.wallet);
    
    // Check current allowance
    const currentAllowance = await tokenContract.allowance(this.wallet.address, this.chainConfig.intent);
    
    // If allowance is already sufficient, return early
    if (currentAllowance >= amount) {
      console.log(`Approval already exists for ${ethers.formatUnits(currentAllowance)} tokens`);
  ...
  async initiateTransfer(params: InitiateTransferParams): Promise<string> {
    ...
    const receiverBytes = ethers.getBytes(ethers.zeroPadValue(params.receiver, 20));
