...
import { CHAINS, CURRENT_NETWORK, POLL_INTERVAL_MS, MAX_POLL_ATTEMPTS } from './constants';
...
async function main() {
  ...
  try {
    ...
    const arbitrumUsdcBalance = await arbitrumClient.getTokenBalance(arbitrumConfig.usdc);
    ...
    if (finalIntent.status === 'fulfilled' || finalIntent.status === 'settled') {
      console.log('Success! The intent was processed successfully.');
      
      // Check final balances
      const finalArbitrumUsdcBalance = await arbitrumClient.getTokenBalance(arbitrumConfig.usdc);
      ...
      console.log(`The intent is still in ${finalIntent.status} state after maximum polling attempts.`);
    }
